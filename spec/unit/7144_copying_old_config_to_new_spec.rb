...
describe 'Clone' do
  ...
  it '#clone should exit with non-zero code if nodes in old config are not existing' do
    ...
    lambda{ copy_old_config_to_new(EMPTY_FOLDER, VALID_NEW_PATH) }.should raise_error(RuntimeError, "In old config directory spec/unit/test_empty_folder nodes are not found")
    FileUtils.rm_rf(EMPTY_FOLDER)
  end

  it '#clone should exit with non-zero code if new folder is existing' do
    FileUtils.mkdir(EMPTY_FOLDER)
    lambda{ copy_old_config_to_new(VALID_OLD_PATH, EXISTING_NEW_PATH) }.should raise_error(RuntimeError, "New config directory spec/unit/test_empty_folder is existing")
    FileUtils.rm_rf(EMPTY_FOLDER)
  end
