...
describe 'Clone' do
  ...
  it '#clone should exit with non-zero code if path to old config is not existing' do
    lambda{ Clone.new.copy_old_config_to_new(OLD_NOT_EXISTING_PATH, VALID_NEW_PATH) }.should  raise_error(RuntimeError, 'pathToOldNotExistingConfig: old config is not fully created')
  end

  it '#clone should exit with non-zero code if nodes in old config are not existing' do
    FileUtils.mkdir(EMPTY_FOLDER)
    lambda{ Clone.new.copy_old_config_to_new(EMPTY_FOLDER, VALID_NEW_PATH) }.should raise_error(RuntimeError, 'spec/unit/test_empty_folder: old config is not fully created')
  ...
  it '#clone should exit with non-zero code if new folder is existing' do
    ...
    lambda{ Clone.new.copy_old_config_to_new(VALID_OLD_PATH, EXISTING_NEW_PATH) }.should raise_error(RuntimeError, 'spec/unit/test_empty_folder: new config directory already exists (remove it and try again)')
