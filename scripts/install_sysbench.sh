...
if [[ "0" -ne "$run_command_ret_val" ]]; then
        run_command 'sudo apt-get install  build-essential libtool autotools-dev -y'
	run_command 'sudo yum -y groupinstall "Development Tools"'
        ...
        run_command 'sudo zypper --non-interactive install libtool automake autoconf'
fi
run_command 'cd sysbench && ./autogen.sh'
run_command 'mysql -V'
if [[ "0" -ne "$run_command_ret_val" ]]; then
        echo 'MariaDB or MySQL are not installed'
	exit 1
fi
run_command 'cd sysbench && ./configure'
run_command 'cd sysbench && make'
run_command 'cd sysbench && make install'
